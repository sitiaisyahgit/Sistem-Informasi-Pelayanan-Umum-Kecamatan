...
class Legalisasi extends CI_Controller {
	...
	function index() {
		...
		$this->load->view('pegawai/v_legalisasi', $x);
	}
	...
	function get_legalisasi_byid() {
		...
		$agama = strip_tags($this->input->post('xagama')); 
		$alamat = strip_tags($this->input->post('xalamat'));
		$this->m_legalisasi->get_legalisasi_byid($nomor_legalisasi, $tanggal_buat, $nama, $nik, $tempat_tl, $tanggal_lahir, $jenis_kelamin, $warganegara, $agama, $alamat);
		echo $this->session->set_flashdata('msg', 'success');
		redirect('pegawai/legalisasi');
	}
