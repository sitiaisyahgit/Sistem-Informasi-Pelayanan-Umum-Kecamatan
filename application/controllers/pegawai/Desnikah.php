...
class Desnikah extends CI_Controller {
	...
	function index() {
		...
		$this->load->view('pegawai/v_desnikah', $x);
	}
	...
	function get_desnikah_byid() {
		...
		$agama_pr = strip_tags($this->input->post('xagama_pr')); 
		$status_pr = strip_tags($this->input->post('xstatus_pr'));  
		$this->m_desnikah->get_desnikah_byid($nomor_desnikah, $tanggal_buat, $nama_pria, $tempat_tl_pria, $tanggal_lahir_pria, $jenis_kelamin_pria, $agama_pria, $status_pria, $nama_pr, $tempat_tl_pr, $tanggal_lahir_pr, $jenis_kelamin_pr, $agama_pr, $status_pr);
		echo $this->session->set_flashdata('msg', 'success');
		redirect('pegawai/desnikah');
	}
